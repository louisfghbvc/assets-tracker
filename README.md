...
*   **資產統計圖 (Stats Tab)**：圓餅圖顯示投資組合市場分佈（台股 / 美股 / 加密貨幣）；切換至「By Asset」模式後每檔資產獨立一塊，依市場自動上色並分組顯示。
*   **績效分析 (Performance Tab)**：專屬「績效」頁籤顯示投資組合年化報酬率 (CAGR)、持有天數，以及與 TAIEX (^TWII) 和 S&P 500 (SPY) 的基準比較；各資產 CAGR 排行讓你一眼看出最佳與最差表現，同一檔股票多批買入自動合併計算。
*   **最新財經新聞 (News Tab)**：顯示持股市值前 10 大標的的最新新聞標題，包含文章來源與發布時間（例：2h ago）。需設定 `VITE_CORS_PROXY_URL` 才能使用，未設定時頁籤會顯示提示說明。
